...
if [ -f /.dockerenv ]; then
    ...
    case "$COMMAND" in
        ...
        history)
            ...
            ;;
        *)
            echo "Usage: $0 {revision|upgrade|downgrade|current|history} [message]"
            exit 1
            ;;
    esac
else
    # Outside container — use docker-compose to run inside API container
    CONTAINER="api"
    DOCKER_CMD="docker compose -f docker-compose.dev.yml exec $CONTAINER bash -lc"
